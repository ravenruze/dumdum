...
<head>
    ...
    <title>Edit Data Sapi - Istana Qurban</title>
    ...
    <form action="{{ route('sapi.update', $sapi->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="f-row">
            ...
            <select name="status" class="f-input">
                <option value="Tersedia" {{ old('status', $sapi->status) == 'Tersedia' ? 'selected' : '' }}>AVAILABLE</option>
                <option value="Booking" {{ old('status', $sapi->status) == 'Booking' ? 'selected' : '' }}>BOOKED</option>
                <option value="Terjual" {{ old('status', $sapi->status) == 'Terjual' ? 'selected' : '' }}>SOLD</option>
            </select>
        </div>
